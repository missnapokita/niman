...
const { hasUnlock, consumeUnlockHeaders } = require("./_gate-lib");
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (result && result.consumeGate) {
      const gateHeaders = consumeUnlockHeaders(req);
      if (gateHeaders) res.setHeader("Set-Cookie",gateHeaders);
    }
