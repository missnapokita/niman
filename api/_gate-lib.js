...
const USED_COOKIE = "bidamax_gate_used";
...
function sign(payload) {
  ...
  const sig = base64url(crypto.createHmac("sha256",gateSecret()).update(body).digest());
  return body + "." + sig;
}
...
function readUnlock(req) {
  ...
  if (!token || token.kind !== "unlock") return null;

  const used = verify(c[USED_COOKIE]);
  if (used && used.kind === "used" && used.nonce === token.nonce) return null;

  return token;
}

function hasUnlock(req) {
  return !!readUnlock(req);
}

function consumeUnlockHeaders(req) {
  const token = readUnlock(req);
  ...
  return [
    ...
    makeCookie(USED_COOKIE,used,10 * 60)
  ];
}
