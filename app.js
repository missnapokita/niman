(function(){
  ...
  async function enforceGateWhenNeeded(){
    // While the current 2-day voucher is active, refresh/reopen stays here.
  ...
  // No active voucher = shortlink gate required.
  (async function(){
    var restored = await restoreFromServer();

    if(!restored){
      await enforceGateWhenNeeded();
    }
  })();
